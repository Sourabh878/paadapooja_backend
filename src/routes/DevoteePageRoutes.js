...
router.post('/', async (req, res) => {
  try {
    ...
    const response = await fetch('https://api.skitechno.com/api/PGM/get_devotee', {
      method: 'POST',
      headers: {
...
router.post('/book-seva', async (req, res) => {
    try {
        ...
        const hasA = sevas.some(s => s.name.toLowerCase().includes('panchamruta'));
        ...
        // --- 4. SEVA MAPPING ---
        let seva_1 = null, seva_1_amount = 0;
        let seva_2 = null, seva_2_amount = 0;

        sevas.forEach(s => {
            if (s.name.toLowerCase().includes('paadha pooja')) {
                seva_1 = 'Paadha Pooja'; seva_1_amount = s.amount;
            } else if (s.name.toLowerCase().includes('panchamruta')) {
                seva_2 = 'Panchamruta'; seva_2_amount = s.amount;
            }
        });
        ...
        const result = await pool.query(insertQuery, [
            devotee_id, devotee_name, devotee_address, devotee_mobile,
            final_token_number, final_token_prefix, final_token,
            seva_1, seva_1_amount, seva_2, seva_2_amount,
...
router.get('/', async (req, res) => {
    try {
        const query = `
            SELECT * FROM dummy_seva_bookings where status=0
...
router.put('/update-amounts/:id', async (req, res) => {
    try {
        ...
        const result = await pool.query(updateQuery, [donation, GD, BD,status,id]);
