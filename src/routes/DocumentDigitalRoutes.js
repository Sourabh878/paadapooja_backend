 ...
 */
router.post('/upload', authenticate,upload.array('images'),async (req, res) => {
    ...
    try {
        ...
        const docRegistryQuery = `
            ...
            RETURNING doc_id
        `;
        const docResult = await client.query(docRegistryQuery, [document_name, remarks, branch_id]);
        ...
        res.status(201).json({ message: "Document digitalised successfully", doc_id: docId });
...
router.delete('/:id', authenticate, async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: "Document and all scans deleted successfully" });
    } catch (err) {
...
router.post('/images/addImage/:id',authenticate,upload.array('images'),async (req, res) => {
