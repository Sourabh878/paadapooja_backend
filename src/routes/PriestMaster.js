...
router.get('/',authenticate, async (req, res) => {
    try {
        ...
        const result = await pool.query('SELECT * FROM priest_master ORDER BY id DESC');
...
router.post('/',authenticate, upload.single("image"), async (req, res) => {
    try {
        ...
        const { name, k_name, address, k_address, city, pincode, phone_number, email, education, university, gotra, kuladevata,aadhar_number,pan_number,dl_number } = req.body;
        ...
        const result = await pool.query(
            ...
            [name, k_name, address, k_address, city, pincode, phone_number, email, education, university, gotra, kuladevata, aadhar_number, pan_number, dl_number, image_url]
        );
...
router.put('/:id',authenticate, upload.single("image"), async (req, res) => {
    try {
        ...
        const existingPriest = await pool.query('SELECT photo_url FROM priest_master WHERE id = $1', [id]);
        if (existingPriest.rows.length === 0) return res.status(404).json({ error: "Priest not found" });
        ...
        const result = await pool.query(
            ...
            [name, k_name, address, k_address, city, pincode, phone_number, email, education, university, gotra, kuladevata, image_url, aadhar_number, pan_number, dl_number, id]
        );
